...
var     t1$1 = new Date;
    ...
    var Chart = (function () {
        function Chart(selector, domain, data) {
            ...
            var xAxisGrid = axisBottom(this.xScale)
                .tickSize([this.width]);
            var yAxis = axisLeft(this.yScale);
            var yAxisGrid = axisLeft(this.yScale)
                .tickSize([this.width]);
            // Draw Axises
            var xAxisGroup = this.svg
                .append('g')
            ...
            this.svg.on('click', (function () {
                var that = _this;
                return function () {
                    var _a = mouse(this), x = _a[0], y = _a[1];
                    if (that.mode) {
                        that.addPoint(x, y);
                    }
                };
            ...
            this.dragBehavior = drag()
                ...
                .on("start", function () { return _this.dragStarted(); })
        ...
        Chart.prototype.toggleMode = function () {
            this.mode = !this.mode;
            if (!this.mode) {
                this.svg
        ...
        Chart.prototype.dragEnded = function () {
            var _this = this;
            if (!this.mode) {
                var _a = [event.x, event.y], x = _a[0], y = _a[1];
        ...
        Chart.prototype.updateTrainingLine = function (slope, offset) {
            ...
            var minY;
            var maxX = this.xScale(maxScaledX);
            var maxY;
            if (slope === Infinity) {
                minY = 0;
                minX = 0,
    ...
    var vis = new Chart('.visual-container', [-50, 50], []);
    $('#reset')
        .on('click', function () {
        vis.reset();
    });
    ...
    $("#train")
        .on('click', function () {
        var slope = (Math.round(10 * Math.random()) - 5) / 2;
        var offset = Math.round(40 * Math.random()) - 20;
        vis.updateTrainingLine(slope, offset);
    });
    var element = document.querySelector('.visual-container');
    var $element = $('.visual-container');
    $element
        .on('pointAdded', function (event) {
        console.log('pointAdded', event.detail);
    });
    element
        .addEventListener('idealLineUpdated', function (event) {
        console.log('idealLineUpdated', event.detail);
    });
    element
        .addEventListener('trainingLineUpdated', function (event) {
